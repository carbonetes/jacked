...
// Scan scans the Go BOM for vulnerabilities and returns a slice of cyclonedx.Vulnerability.
func (s *Scanner) Scan(bom *cyclonedx.BOM) ([]cyclonedx.Vulnerability, error) {
	...
	for _, c := range *bom.Components {
		...
		if helper.GetComponentType(c.Properties) != "go" {
			continue
		}
		...
		goVersion, err := version.NewGoVersion(c.Version)
		if err != nil {
			continue
		}
		...
		for _, vuln := range *vulns {
			...
			match, err := goVersion.Check(vuln.Constraints)
			if err != nil || !match {
				continue
			}
