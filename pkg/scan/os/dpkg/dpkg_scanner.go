package dpkg

import (
	"github.com/CycloneDX/cyclonedx-go"
	"github.com/carbonetes/jacked/internal/db"
	"github.com/carbonetes/jacked/internal/helper"
	"github.com/carbonetes/jacked/internal/log"
	v3 "github.com/carbonetes/jacked/pkg/model/cdx"
	"github.com/carbonetes/jacked/pkg/version"
)

// dpkgScanner implements scan.Scanner for dpkg packages.
type Scanner struct {
	store db.Store
}

// NewScanner creates a new dpkgScanner with the given db.Store.
func NewScanner(store db.Store) *Scanner {
	return &Scanner{
		store: store,
	}
}

// Scan implements the scan.Scanner interface.
func (s *Scanner) Scan(bom *cyclonedx.BOM) ([]cyclonedx.Vulnerability, error) {
	var results []cyclonedx.Vulnerability
	if bom == nil || bom.Components == nil || len(*bom.Components) == 0 {
		return results, nil
	}

	for _, c := range *bom.Components {
		if c.Properties == nil {
			continue
		}
		if helper.GetComponentType(c.Properties) != "deb" {
			continue
		}
		vulns := s.store.DebSecTrackerMatch(c.Name)
		upstream := helper.FindUpstream(c.BOMRef)
		if upstream != "" {
			upstreamVulns := s.store.DebSecTrackerMatch(upstream)
			if upstreamVulns != nil {
				if vulns == nil {
					vulns = upstreamVulns
				} else {
					*vulns = append(*vulns, *upstreamVulns...)
				}
			}
		}

		if vulns == nil {
			continue
		}

		dpkgVersion, err := version.NewDpkgVersion(c.Version)
		if err != nil {
			continue
		}
		for _, vuln := range *vulns {
			if vuln.Constraints == "" {
				continue
			}

			match, err := dpkgVersion.Check(vuln.Constraints)
			if err != nil {
				log.Debugf("error checking dpkg version %s against constraint %s: %v", c.Version, vuln.Constraints, err)
				continue
			}

			if match {
				vex := v3.ToVex(&vuln, &c, vuln.Constraints)
				if vex != nil {
					results = append(results, *vex)
				}
			}
		}
	}
	return results, nil
}
